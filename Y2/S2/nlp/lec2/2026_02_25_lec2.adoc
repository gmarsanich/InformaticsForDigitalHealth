...
CAUTION: F1 can be a TRAP

F1 is the harmonic mean of precision and recall -> assumes precision and recall have equal importance.

* in some contexts one is more important than the other
** medical diagnosis: low recall is worse than low precision
** spam detection: false positives are worse than false negatives
...
CAUTION: can also be a trap

Generalized form of F1

* assigns weights (stem:[\beta]) to recall and precision
* stem:[\beta] needs to be chosen carefully
** also depends on specific task and application
* also ignores True Negatives
** can be misleading for imbalanced datasets
...
*** model can just predict the majority class and get it mostly correct -> high accuracy but useless
