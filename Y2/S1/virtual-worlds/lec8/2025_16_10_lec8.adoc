...
.Definition
"Reprojection error is a measure of the discrepancy between the predicted image positions based on perspective projection geometry and the actual observed image positions. It is obtained by constructing a camera imaging model using internal and external parameters, distortion coefficient, and re-projecting three-dimensional points in space. The error is defined as the mean Euclidean distance between the image coordinates of the projected control points and the image coordinates of the detected control points. Reprojection error is used to evaluate the accuracy of camera pose estimation in the image domain."
...
.Definition
"A process used in the reconstruction of a three-dimensional model from a set of photographs, simultaneously refining the coordinates describing the scene geometry as well as the parameters of the relative motion and the optical characteristics of the cameras that took the pictures."
