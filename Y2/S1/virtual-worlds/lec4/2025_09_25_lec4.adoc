...
* `w` grows with the distance from the camera
* when we divide by `w` the distance is removed
** only the direction of the ray that connects the camera to the point is kept
* this gives an `NDC`
** normalized cube
