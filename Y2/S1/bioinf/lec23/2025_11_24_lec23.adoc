...
* cannot bind to lactose so it stays where it is
* blocks DNA polymerase
** blocks transcription of lactose degradation RNA
** target genes are not produced
...
GRNs are *inferred* from data about gene expression

* like by looking for correlation between 2 genes being expressed in differnt conditions
** can infer influence of one on the other
* need to identify *transcription factors*
** what causes specific RNA to be transcribed
...
* *synchronous update*: all genes are updated at the same time
** preferred for simulations
* *asynchronous update*: one gene/subset of nodes is updated at a time
** more realistic
*** gene A might take a while to influence gene B
*** gene C might be able to influence gene D instantly

The description of all possible steps in a graph is called the *State Transition Graph*
...
Attractor analysis is used to:

* determine the attractors
* determine their basins of attraction
* determine/infer their biological meaning

=== Attractors in Synchronous Dynamics

They're pretty trivial in this case

Attractors are just a set of states that the system iterates on

Synchronous basins are disjoint from each other

* meaning that each basin is unique
** one state cannot appear in more than one basin
...
Asynchronous basins may overlap depending on nondeterminism

* which means that there may be states that appear in more than one basin
...
image::attractor.png[attractor graph, 320, 240, align="center"]
...
* i.e. we end up with 2 attractors of size 1

How we get to them depends on the STG
