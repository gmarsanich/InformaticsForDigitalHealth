...
.Naive/Brute force
* parse the sequence
* parse the search pattern for every character in the sequence
** extremely high complexity
** at worst have to parse every charcter in the pattern for every character in the sequence
*** `O(s * p)` complexity

.Knuth-Morris-Pratt (KMP) algorithm
. uses preprocessing step to skip some comparisons
...
IUPAC tables assign codes to:

* nucleotides
** some are direct matches
*** `IUPAC(A) = "Adenine"; nucleotide("Adenine") = "A"`
** some are not
*** `IUPAC(B) = "not A"; nucleotide("not A") = {"C", "G", "T"}`
* amino acids
** `IUPAC(A) = "Ala"; aacid("Ala") = "Alanine`
