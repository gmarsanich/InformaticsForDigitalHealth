...
Gillespie's stochasti simulation algorithm (SSA)
...
* X = time at which reaction will take place
* we want to compute the probability that X > t+s
* we know the reaction will happen for sure after s -> X > s
* the probability of -> X > t
** this means that from time s we can generate a new time and we don't care about the past
