...
Sometimes we have more than 2 strings we want to align
...
[stem]
++++
\text{Score} = sm(A, A) + g) + 3 ⋅ sm(R, R) + (sm(A, C) + g) + 3 ⋅ sm(D,D)+ 2 ⋅ g + 2 ⋅ sm(S, A) + sm(A, A)
++++
...
More robust version of CLUSTAL that reduces error propagation
